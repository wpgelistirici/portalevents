...
export default function ArtistRequestsPage() {
  ...
  const filtered = artistTickets.filter((at) => {
    ...
    const matchSearch =
      !search ||
      at.artistName.toLowerCase().includes(q) ||
      at.genre.toLowerCase().includes(q);
    ...
    <div className="space-y-6">
      ...
      {filtered.length === 0 ? (
        <div className="text-center py-16">
          ...
          <p className="text-foreground/40">{t("noRequests")}</p>
        </div>
      ) : (
        <div className="space-y-3">
          {filtered.map((at) => (
            <div
              key={at.id}
              className="flex items-center gap-4 p-5 rounded-2xl bg-foreground/5 border border-foreground/10 backdrop-blur-xl"
            >
              <div
                className={`w-10 h-10 rounded-xl flex items-center justify-center flex-shrink-0 ${at.status === "open" ? "bg-amber-500/10" : "bg-emerald-500/10"}`}
              >
                {at.status === "open" ? (
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2">
                  ...
                  <span
                    className={`text-[10px] px-2 py-0.5 rounded-full font-medium ${at.status === "open" ? "bg-amber-500/10 text-amber-400 border border-amber-500/20" : "bg-emerald-500/10 text-emerald-400 border border-emerald-500/20"}`}
                  >
                    {at.status === "open"
                      ? t("statusOpen")
                      : t("statusResolved")}
                ...
                <p className="text-sm text-foreground/40 mt-0.5">
                  {at.genre} · {at.description}
                </p>
              </div>
              <span className="text-xs text-foreground/20">
                {new Date(at.createdAt).toLocaleDateString(locale)}
              </span>
            </div>
          ))}
        </div>
