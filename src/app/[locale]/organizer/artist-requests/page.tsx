    ...
    <div className="space-y-6">
      ...
      {filtered.length === 0 ? (
        ...
        <div className="space-y-3">
          {filtered.map((at) => (
            <div key={at.id} className="flex items-center gap-4 p-5 rounded-2xl bg-foreground/5 border border-foreground/10 backdrop-blur-xl">
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2">
                  <span className="text-foreground font-semibold">{at.artistName}</span>
                ...
                <p className="text-sm text-foreground/40 mt-0.5">{at.genre} · {at.description}</p>
              </div>
              <span className="text-xs text-foreground/20">{new Date(at.createdAt).toLocaleDateString("tr-TR")}</span>
            </div>
          ))}
        </div>
      )}
