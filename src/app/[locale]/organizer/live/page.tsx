    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          ...
          <button onClick={toggleFullscreen} className="p-3 rounded-xl bg-foreground/5 border border-foreground/10 text-foreground/60 hover:text-foreground hover:bg-foreground/10 transition-colors">
      ...
      {!selectedEventId ? (
        <div className="text-center py-24">
          <Radio className="w-16 h-16 text-foreground/10 mx-auto mb-4" />
          <p className="text-foreground/40 text-lg">{t("selectPrompt")}</p>
          <p className="text-foreground/20 text-sm mt-1">{t("selectPromptDesc")}</p>
        </div>
      ) : eventData ? (
        <div className="space-y-6">
          {/* Main Progress */}
          <div className="rounded-2xl bg-foreground/5 border border-foreground/10 backdrop-blur-xl p-8">
            <div className="text-center mb-6">
              <p className="text-foreground/50 text-sm">{eventData.event?.title}</p>
            ...
            <div className="text-center mb-8">
              <p className="text-7xl font-black text-foreground">{eventData.checkedIn}</p>
              <p className="text-foreground/40 mt-1">/ {eventData.totalQty} {t("checkedIn")}</p>
            </div>

            {/* Progress bar */}
            <div className="h-6 rounded-full bg-foreground/5 overflow-hidden mb-4">
              <motion.div
                initial={{ width: 0 }}
            ...
            <p className="text-center text-foreground/50 text-sm">{eventData.checkInRate.toFixed(1)}% {t("checkInRate")}</p>
          </div>
          ...
          <div className="grid grid-cols-3 gap-4">
            {[
              ...
              <div key={i} className="rounded-2xl bg-foreground/5 border border-foreground/10 backdrop-blur-xl p-5 text-center">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div className="rounded-2xl bg-foreground/5 border border-foreground/10 backdrop-blur-xl p-6">
              <h3 className="text-sm font-semibold text-foreground mb-4">{t("byTicketType")}</h3>
                  ...
                  return (
                    <div key={tt.name}>
                      <div className="flex items-center justify-between text-sm mb-1">
                        ...
                        <span className="text-foreground/40">{tt.used}/{tt.total}</span>
                      </div>
            ...
            <div className="rounded-2xl bg-foreground/5 border border-foreground/10 backdrop-blur-xl p-6">
              ...
              {eventData.recentLogs.length === 0 ? (
                <p className="text-foreground/30 text-sm py-4 text-center">{t("noEntries")}</p>
              ) : (
                <div className="space-y-2 max-h-64 overflow-y-auto scrollbar-hide">
                  {eventData.recentLogs.map((log) => (
                    <div key={log.id} className="flex items-center gap-3 py-2 px-3 rounded-lg bg-foreground/[0.02]">
                      <CheckCircle2 className="w-4 h-4 text-emerald-400 flex-shrink-0" />
                      <div className="flex-1 min-w-0">
                        <p className="text-sm text-foreground truncate">{log.ticketHolderName}</p>
                        <p className="text-[10px] text-foreground/30">{log.ticketType}</p>
                      </div>
