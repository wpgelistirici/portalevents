...
export default function LiveCheckinPage() {
  ...
  const eventOptions = approvedEvents.map((e) => ({
    value: e.id,
    label: e.title,
  }));
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          ...
          <button
            ...
            className="p-3 rounded-xl bg-foreground/5 border border-foreground/10 text-foreground/60 hover:text-foreground hover:bg-foreground/10 transition-colors"
          >
            {isFullscreen ? (
        ...
        <div className="space-y-6">
          ...
          <div className="rounded-2xl bg-foreground/5 border border-foreground/10 backdrop-blur-xl p-8">
            ...
            <div className="text-center mb-8">
              ...
              <p className="text-foreground/40 mt-1">
                / {eventData.totalQty} {t("checkedIn")}
              </p>
            </div>
            ...
            <p className="text-center text-foreground/50 text-sm">
              {eventData.checkInRate.toFixed(1)}% {t("checkInRate")}
            </p>
          </div>
          ...
          <div className="grid grid-cols-3 gap-4">
            {[
              ...
              {
                label: t("entered"),
                value: eventData.checkedIn,
                icon: CheckCircle2,
                color: "text-emerald-400",
                bg: "bg-emerald-500/10",
              },
              {
                label: t("cancelledTickets"),
                value: eventData.cancelled,
                icon: XCircle,
                color: "text-red-400",
                bg: "bg-red-500/10",
              },
              ...
              <div
                key={i}
                className="rounded-2xl bg-foreground/5 border border-foreground/10 backdrop-blur-xl p-5 text-center"
              >
                <div
                  className={`w-10 h-10 rounded-xl ${kpi.bg} flex items-center justify-center mx-auto mb-3`}
                >
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div className="rounded-2xl bg-foreground/5 border border-foreground/10 backdrop-blur-xl p-6">
              ...
              {eventData.recentLogs.length === 0 ? (
                <p className="text-foreground/30 text-sm py-4 text-center">
                  {t("noEntries")}
                </p>
              ) : (
                <div className="space-y-2 max-h-64 overflow-y-auto scrollbar-hide">
                  {eventData.recentLogs.map((log) => (
                    <div
                      key={log.id}
                      className="flex items-center gap-3 py-2 px-3 rounded-lg bg-foreground/[0.02]"
                    >
