        ...
        <div className="grid grid-cols-7 gap-1">
          {cells.map((day, i) => {
            ...
            return (
              <div
                key={i}
                className={`min-h-[80px] rounded-xl p-2 ${isToday ? "bg-[#7B61FF]/10 border border-[#7B61FF]/20" : "bg-foreground/[0.02] border border-foreground/5"}`}
              >
                <span
                  className={`text-xs ${isToday ? "text-[#7B61FF] font-bold" : "text-foreground/40"}`}
                >
                  {day}
                </span>
                <div className="mt-1 space-y-1">
                  {dayEvents.map((ev) => (
                    <div
                      key={ev.id}
                      className="flex items-center gap-1.5 px-1.5 py-1 rounded bg-foreground/[0.04]"
                    >
        ...
        {venueEvents.length === 0 ? (
          ...
          <div className="space-y-2">
            {venueEvents.map((ev) => (
              <div
                key={ev.id}
                className="flex items-center gap-4 p-3 rounded-xl bg-foreground/[0.02]"
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-foreground/40">
                    {ev.artist} · {ev.date} · {ev.time}
                  </p>
                </div>
                <span
                  className={`text-[10px] px-2 py-0.5 rounded-full ${ev.status === "approved" ? "bg-emerald-500/10 text-emerald-400" : "bg-foreground/5 text-foreground/40"}`}
                >
                  {ev.status}
                </span>
              </div>
